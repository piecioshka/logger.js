...
buster.testCase("logger/dom DOM Core Level 3", {
    ...
    "[??? DOMUserData]": function () { assert(false); },

    // 1.2.4 The DOMObject Type
    "[??? DOMObject]": function () { assert(false); },

    // 1.4 Fundamental Interfaces: Core Module

    // exceptions
    "[exception DOMException]": function () { assert(false); },

    // interfaces
    "[interface DOMStringList]": function () { assert(false); },
    "[interface NameList]": function () { assert(false); },
    "[interface DOMImplementationList]": function () { assert(false); },
    "[interface DOMImplementationSource]": function () { assert(false); },
    ...
    "[interface TypeInfo]": function () { assert(false); },
    "[interface UserDataHandler]": function () { assert(false); },
    "[interface DOMError]": function () { assert(false); },
    "[interface DOMErrorHandler]": function () { assert(false); },
    "[interface DOMLocator]": function () { assert(false); },
    "[interface DOMConfiguration]": function () { assert(false); },
